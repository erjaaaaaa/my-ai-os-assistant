...
- 2026-09-26 — **A content diff was used to justify discarding a git ref, and the reachability proof was run afterwards** — the 12:00 UTC hourly run met the familiar Step 0 § 0 container shape (detached `HEAD` == `origin/main` == `21e379b`, tree clean, `git pull` reporting `+ fb6b48b...21e379b (forced update)`, local `main` on the graft lineage), followed § 0 as written, and hit *refusing to merge unrelated histories*. Instead of the established **(e)+(a)** recovery it ran **`git checkout -B main origin/main`** — option (b) of `6hXqrrHRVQHjvg9Q`, the only option that *discards* a ref, and one an earlier run had explicitly placed outside the write allowlist absent Eriks's say-so. **The justification it used was `git diff --numstat main origin/main`**, which showed `origin/main` adding 1013 lines and removing nothing but the two watermark lines. That reasoning is unsound in a way worth naming precisely: **a content diff answers "is the remote's tree a superset?", which is not the question. The question is "are the commits I am about to drop reachable from the remote?", and only ancestry answers it.** Two trees can agree on content while the refs sit on unrelated histories — which is exactly what a shallow graft makes *look* true. The proof that was available (2026-09-25, on this same task) was run **after** the discard: `--is-shallow-repository` **true** → `git fetch --unshallow origin main` → **false** → `git merge-base --is-ancestor fb6b48b origin/main` **succeeds**. It came back clean — `fb6b48b` was a plain ancestor, nothing left the repository, eleventh container in which the "unrelated histories" is the graft and not a fork — **but a safety check that runs after the irreversible act is not a safety check, it is a postmortem that happened to be favourable.** **Rule:** never discard or re-point a ref on the strength of a *content* comparison; the only admissible evidence is `git merge-base --is-ancestor <old-ref> origin/main`, run **after** unshallowing and **before** the discard, with the result recorded. And generally: when a run substitutes a cheaper check for the prescribed one, the substitution is the defect even when the outcome is clean — report it as a defect of the run rather than as a successful recovery, because the clean outcome is the single least informative thing about it. Sibling of the 2026-09-26 05:00 entry (a guard that does not short-circuit the mutation is decoration): there the right check was wired so it could not bind, here the right check was run too late to bind. Both are the same failure — **a check is only a check if the mutation cannot happen when it fails.** Extends, does not replace, the 2026-09-25 ancestry entry; its rule is unchanged and is precisely what this run got backwards.
