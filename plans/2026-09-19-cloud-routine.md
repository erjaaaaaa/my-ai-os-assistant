...
| 13 | First laptop run after the cloud run: pull, confirm Steps 1–2 no-op and Step 3 catches up | assistant | open — now the normal shape of every `/start-day`, which Eriks runs on the laptop on demand (2026-09-20) |
| 14 | Decide the vault: keep Step 3 on the laptop (default) or push the vault to a private repo and run Step 3 in the cloud | **Eriks** | **asked for 2026-09-20** — *"Can we move it online and then the local obsidian catches up with whatever was advanced online?"* Design in § Vault online below; the deferral stays until 14a–14d are done. **Decided 2026-09-20:** *"Ok, let's keep the vault local."* Design parked; Step 3 stays on the laptop |
| 15 | Decide delivery: commit only (current) or a push channel, which needs a written widening of the security boundary | **Eriks** | moot 2026-09-20 — the brief is a laptop deliverable again (step 21) |
...
| 21 | Daily cloud routine **disabled**, hourly cron widened to every hour (server stored `59 * * * *`), hourly prompt reworded ("Eriks runs /start-day on the laptop for those"). Eriks 2026-09-20: *"Let there be inbox sweeps every hour, but remove the daily brief from the cloud run. I will just run /start-day myself here on the laptop whenever I'm ready."* | assistant | done 2026-09-20 — both routines read back |
| 22 | Step 3's candidate query written down in `procedures/step-3-ingest.md` § Finding mail to snapshot (all mail newer than the watermark, not the inbox) | assistant | done 2026-09-20 |
| 20 | Model for the hourly sweep: Opus (current, parity with the daily) or a cheaper model — up to 23 extra Opus runs a day | **Eriks** | decided 2026-09-20 — *"Opus is fine for now."* Stays `claude-opus-5`; revisit only if Eriks raises it |

## Vault online — design for step 14 (proposed 2026-09-20, **parked the same day** — Eriks: *"Ok, let's keep the vault local."*)
